...
public class InboundController {
    public void download(AppRepository model, BaseViewModel viewModel, SyncInfo syncInfo, SyncItemViewModel.SyncInfoUpDownLoadListener listener) {
        ...
        if (syncDate != null) {
            ...
            if (inboundDatas != null && inboundDatas.size() > 0) {
                // 存在未上传的数据，弹窗提示
                MaterialDialogUtils.showBasicDialog(viewModel.getApplication(), viewModel.getApplication().getResources().getString(R.string.sync_delete_data_hint_text))
                        .onPositive((dialog, which) -> {
                            toUpload(model, viewModel.getLifecycleProvider(), syncInfo, listener, true);
                        })
                        .onNegative((dialog, which) -> {
                            toDownload(model, viewModel.getLifecycleProvider(), syncInfo, listener);
                        })
                        .show();
                return;
            }
