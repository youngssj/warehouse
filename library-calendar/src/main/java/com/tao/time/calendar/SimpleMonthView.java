...
class SimpleMonthView extends View {
    ...
    public SimpleMonthView(Context context, TypedArray typedArray, DayPickerView.DataModel dataModel) {
        ...
        //获取字体
        mDayOfWeekTypeface = resources.getString(R.string.calendar_sans_serif);
        mMonthTitleTypeface = resources.getString(R.string.calendar_sans_serif);

        //今天字体下小圆圈颜色
        mCurrentDayColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorRoundDay, Color.BLUE);
        //今天字体颜色
        mCurrentDayTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorCurrentDay, resources.getColor(R.color.calendar_to_day));
        //头部年份月份字体颜色
        mYearMonthTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorYearMonthText, resources.getColor(R.color.calendar_years_text));
        //头部星期几字体颜色
        mWeekTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorWeekText, resources.getColor(R.color.calendar_week_day));
        //正常日期颜色
        mDayTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorNormalDayText, resources.getColor(R.color.calendar_normal_day));
        //底部文字的字体颜色
        mTagTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorTagText, resources.getColor(R.color.calendar_bottom_normal_day));
        //已过去的日期字体颜色
        mPreviousDayTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorPreviousDayText, resources.getColor(R.color.calendar_formerly_day));
        //被选中的日期背景颜色
        mSelectedDaysBgColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorSelectedDayBackground, resources.getColor(R.color.calendar_selected_day_background));
        //被选中的日期字体颜色
        mSelectedDayTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorSelectedDayText, Color.WHITE);
        //被占用的日期背景颜色
        mBusyDaysBgColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorBusyDaysBg, Color.TRANSPARENT);
        //被占用的日期字体颜色
        mBusyDaysTextColor = typedArray.getColor(R.styleable.calendar_DayPickerView_calendar_colorBusyDaysText, resources.getColor(R.color.calendar_normal_day));
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);


        //正常日期字体大小
        MINI_DAY_NUMBER_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_textSizeDay, resources.getDimensionPixelSize(R.dimen.calendar_text_size_day));
        //标签字体大小
        TAG_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_textSizeTag, resources.getDimensionPixelSize(R.dimen.calendar_text_size_tag));
        //头部年份月份字体大小
        YEAR_MONTH_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_textSizeYearMonth, resources.getDimensionPixelSize(R.dimen.calendar_text_size_month));
        //头部星期几字体大小
        WEEK_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_textSizeWeek, resources.getDimensionPixelSize(R.dimen.calendar_text_size_day_name));
        //头部高度
        MONTH_HEADER_SIZE = typedArray.getDimensionPixelOffset(R.styleable.calendar_DayPickerView_calendar_headerMonthHeight, resources.getDimensionPixelOffset(R.dimen.calendar_header_month_height));
        //日期半径
        DAY_SELECTED_RECT_SIZE = typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_selectedDayRadius, resources.getDimensionPixelOffset(R.dimen.calendar_selected_day_radius));
        //行高
        mRowHeight = ((typedArray.getDimensionPixelSize(R.styleable.calendar_DayPickerView_calendar_calendarHeight, resources.getDimensionPixelOffset(R.dimen.calendar_calendar_height)) - MONTH_HEADER_SIZE - ROW_SEPARATOR) / 6);
        //已过去的日期是否能被操作
        isPrevDayEnabled = typedArray.getBoolean(R.styleable.calendar_DayPickerView_calendar_enablePreviousDay, false);
        //是否显示标签
        isDisplayTag = typedArray.getBoolean(R.styleable.calendar_DayPickerView_calendar_displayTag, false);
        //获取设置占用日期
...
//        int y = (MONTH_HEADER_SIZE - WEEK_TEXT_SIZE) / 2 + (YEAR_MONTH_TEXT_SIZE / 3);
        int y = MONTH_HEADER_SIZE / 2 + YEAR_MONTH_TEXT_SIZE;
    ...
    protected void drawMonthCell(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            if (dayOffset == 0 || dayOffset == 6) {
                mDayTextPaint.setColor(Color.parseColor("#5d91de"));
            } else {
                mDayTextPaint.setColor(mDayTextColor);
            }
...
//                canvas.drawCircle(x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), 6, mDayTextPaint);
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                ...
                if (isDisplayTag) {
                    ...
                    String lunarTag = LunarUtil.getLunar(cellCalendar.year, cellCalendar.month + 1, cellCalendar.day).lunarDayStr;
                    if (mHasToday && (mToday == day) && isToDayOperation) {
                        mTagTextPaint.setColor(mCurrentDayTextColor);
                    }
                    canvas.drawText(lunarTag, x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                }
                mDayTextPaint.setTextSize(MINI_DAY_NUMBER_TEXT_SIZE);
            ...
            if (isDisplayTag && !isPrevDay && !isBusyDay && !isBeginDay && !isLastDay) {
                ...
                if (!isCalendarTag) {
                    ...
                    canvas.drawText(lunarTag, x, getTextYCenter(mTagTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
                }
            }
...
//                x + DAY_SELECTED_RECT_SIZE, y + DAY_SELECTED_RECT_SIZE);
        RectF rectF = new RectF(x - dp2px(26), y - dp2px(30),
                x + dp2px(26), y + dp2px(20));
        if (isDisplayTag) {
            //绘制圆角矩形
