...
@Route(path = RouterActivityPath.Allocate.PAGER_ALLOCATE_SCAN)
public class AllocateScanActivity extends BaseUhfActivity<AllocateScanActivityBinding, AllocateScanViewModel> {
    ...
    public int initContentView(Bundle savedInstanceState) {
        return R.layout.allocate_scan_activity;
    }
    ...
    @Override
    public AllocateScanViewModel initViewModel() {
        //使用自定义的ViewModelFactory来创建ViewModel，如果不重写该方法，则默认会调用NetWorkViewModel(@NonNull Application application)构造方法
        AppViewModelFactory factory = AppViewModelFactory.getInstance(getApplication());
        return new ViewModelProvider(this, factory).get(AllocateScanViewModel.class);
    }
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            mFragments.add((Fragment) ARouter.getInstance().build(RouterFragmentPath.Allocate.PAGER_ALLOCATE_SCAN).withInt("position", 0).navigation());
            mFragments.add((Fragment) ARouter.getInstance().build(RouterFragmentPath.Allocate.PAGER_ALLOCATE_SCAN).withInt("position", 1).navigation());
        } else {
